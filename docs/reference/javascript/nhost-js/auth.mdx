...
```ts
signInWebAuthn(body?: SignInWebauthnRequest, options?: RequestInit): Promise<FetchResponse<Record<string, unknown>>>;
```
...
`Promise`&lt;[`FetchResponse`](fetch#fetchresponse)&lt;`Record`&lt;`string`, `unknown`&gt;&gt;&gt;

##### signInWebAuthnVerify()

```ts
signInWebAuthnVerify(body: SignInWebauthnVerifyRequest, options?: RequestInit): Promise<FetchResponse<SessionPayload>>;
```
...
```ts
signUpWebAuthn(body: SignUpWebauthnRequest, options?: RequestInit): Promise<FetchResponse<Record<string, unknown>>>;
```
...
```ts
signUpWebAuthnVerify(body: SignUpWebauthnVerifyRequest, options?: RequestInit): Promise<FetchResponse<SessionPayload>>;
```
