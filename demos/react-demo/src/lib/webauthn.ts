 ...
 */
export function prepareRegistrationOptions(publicKeyOptions: any): PublicKeyCredentialCreationOptions {
  ...
  if (credentialOptions.challenge && typeof credentialOptions.challenge === 'string') {
    credentialOptions.challenge = base64URLToArrayBuffer(credentialOptions.challenge);
  }

  // Convert user.id from base64 if necessary
  if (credentialOptions.user?.id && typeof credentialOptions.user.id === 'string') {
  ...
  if (credentialOptions.excludeCredentials) {
    credentialOptions.excludeCredentials = credentialOptions.excludeCredentials.map((credential: any) => {
 ...
 */
export function prepareAuthenticationOptions(publicKeyOptions: any): PublicKeyCredentialRequestOptions {
  ...
  if (credentialOptions.allowCredentials) {
    credentialOptions.allowCredentials = credentialOptions.allowCredentials.map((credential: any) => {
      if (credential.id && typeof credential.id === 'string') {
        return {
          ...credential,
          id: base64URLToArrayBuffer(credential.id),
 ...
 */
export function formatRegistrationCredentialForVerification(credential: PublicKeyCredential): any {
  return {
    id: credential.id,
 ...
 */
export function formatAuthenticationCredentialForVerification(credential: PublicKeyCredential): any {
  return {
    id: credential.id,
    type: credential.type,
    rawId: arrayBufferToBase64URL(credential.rawId),
